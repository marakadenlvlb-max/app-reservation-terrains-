...
describe('DateTimePickerField', () => {
  it('affiche la valeur initiale formatée et masque le sélecteur tant que le champ n\'est pas pressé', () => {
    render(<DateTimePickerField label="Début" value="2026-09-01T18:00" onChange={jest.fn()} />);

    expect(screen.getByText('2026-09-01T18:00')).toBeTruthy();
    expect(screen.queryByTestId('mock-datetimepicker')).toBeNull();
  });

  it('affiche une erreur de validation quand elle est fournie', () => {
    render(<DateTimePickerField label="Début" value="2026-09-01T18:00" onChange={jest.fn()} error="Date invalide" />);
  ...
  describe('sur iOS (mode "datetime" combiné)', () => {
    ...
    it('met à jour la valeur au premier changement puis referme au clic sur "Valider"', () => {
      const onChange = jest.fn();
      render(<DateTimePickerField label="Début" value="2026-09-01T18:00" onChange={onChange} />);

      fireEvent.press(screen.getByLabelText('Début'));
      expect(screen.getByTestId('mock-datetimepicker')).toBeTruthy();

      fireEvent(screen.getByTestId('mock-datetimepicker'), 'change', { type: 'set' }, new Date(2026, 8, 2, 10, 30));
  ...
  describe('sur Android (date puis heure, deux boîtes de dialogue séquentielles)', () => {
    ...
    it("n'appelle onChange qu'une fois la date ET l'heure choisies", () => {
      const onChange = jest.fn();
      render(<DateTimePickerField label="Début" value="2026-09-01T18:00" onChange={onChange} />);
      ...
      fireEvent(screen.getByTestId('mock-datetimepicker'), 'change', { type: 'set' }, new Date(2026, 8, 2, 0, 0));
      expect(onChange).not.toHaveBeenCalled();
      expect(screen.queryByLabelText('Valider la date')).toBeNull();
      expect(screen.getByTestId('mock-datetimepicker')).toBeTruthy();

      // Étape 2/2 : heure choisie — combine avec la date de l'étape 1.
      fireEvent(screen.getByTestId('mock-datetimepicker'), 'change', { type: 'set' }, new Date(2000, 0, 1, 14, 45));

      expect(onChange).toHaveBeenCalledWith('2026-09-02T14:45');
      expect(screen.queryByTestId('mock-datetimepicker')).toBeNull();
    });

    it("n'appelle jamais onChange si l'utilisateur annule la boîte de dialogue", () => {
      const onChange = jest.fn();
      render(<DateTimePickerField label="Début" value="2026-09-01T18:00" onChange={onChange} />);
